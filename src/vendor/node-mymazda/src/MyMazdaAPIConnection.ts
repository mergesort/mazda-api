...
const USER_AGENT_BASE_API = "MyMazda-Android/8.2.0";
const USER_AGENT_USHER_API = "MyMazda/8.2.0 (Google Pixel 3a; Android 11)";
...
export default class MyMazdaAPIConnection {
    ...
    async apiRequestRetry<ResponseType>(needsKeys: boolean, needsAuth: boolean, gotOptions: OptionsOfJSONResponseBody, numRetries: number): Promise<ResponseType> {
        ...
        console.debug(`Sending ${"method" in gotOptions ? gotOptions.method : "GET"} request to ${gotOptions.url}${(numRetries > 0) ? ` - attempt #${numRetries + 1}` : ""}`);
        ...
        try {
            ...
            if (typeof err.message === "string" && err.message.includes("API_ENCRYPTION_ERROR")) {
                console.debug("Server reports request was not encrypted properly. Retrieving new encryption keys.")
                ...
                console.debug("Server reports access token was expired. Retrieving new access token.")
                await this.login();

                return await this.apiRequestRetry(needsKeys, needsAuth, gotOptions, numRetries + 1);
            } else if (typeof err.message === "string" && err.message.includes("LOGIN_ERROR")) {
                console.debug("Login failed for an unknown reason. Trying again.");
                await this.login();

                return await this.apiRequestRetry(needsKeys, needsAuth, gotOptions, numRetries + 1);
            } else if (typeof err.message === "string" && err.message.includes("REQUEST_IN_PROGRESS_ERROR")) {
                console.debug("Request failed because another request was already in progress. Waiting 30 seconds and trying again.");
                await sleep(30000);
    ...
    private async ensureTokenIsValid() {
        if (typeof this.accessToken === "undefined" || this.accessToken.length === 0 || typeof this.accessTokenExpirationTs === "undefined") console.debug("No access token present. Logging in.");
        if (typeof this.accessTokenExpirationTs !== "undefined" && this.accessTokenExpirationTs <= (new Date().getTime() / 1000)) console.debug("Access token is expired. Fetching a new one.");
    ...
    private async retrieveKeys() {
        console.debug("Retrieving encryption keys");
        ...
        console.debug("Successfully retrieved encryption keys");
    ...
    async login() {
        console.debug(`Logging in as ${this.email}`);
        console.debug("Retrieving public key to encrypt password");
        ...
        if (loginResponse.body.status === "INVALID_CREDENTIAL") {
            console.debug("Login failed due to invalid email or password");
            throw new Error("Invalid email or password");
        }

        if (loginResponse.body.status === "USER_LOCKED") {
            console.debug("Login failed to account being locked");
        ...
        console.debug(`Successfully logged in as ${this.email}`);
